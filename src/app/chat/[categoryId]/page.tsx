              ...
              <div className="mt-2 flex gap-4 flex-wrap">
                {[10, 20, 30, 40, 50, 60, 70].map(opt => (
                  <button
                    ...
                    className={`px-4 py-2 border rounded shadow bg-white text-black ${age === opt ? 'bg-gray-100' : ''}`}
                  >
                    {opt}代
                  </button>
                ))}
              ...
              <div className="mt-2 flex gap-4 flex-wrap">
                {[
                  ...
                  <button
                    ...
                    className={`px-4 py-2 border rounded shadow bg-white text-black ${gender === opt.value ? 'bg-gray-100' : ''}`}
                  >
                    {opt.label}
                  </button>
                ))}
              ...
              <div className="mt-2 flex gap-4 flex-wrap">
                {[1, 2, 3, 4, 5].map(opt => (
                  <button
                    ...
                    className={`px-4 py-2 border rounded shadow bg-white text-black ${household === String(opt) ? 'bg-gray-100' : ''}`}
                  >
                    {opt}人
                  </button>
                ))}
                    ...
                    {q.choices.map(choice => (
                      <button
                        ...
                        className={`px-4 py-2 border rounded shadow bg-white text-black ${
                          answers[q.id] === choice.value ? 'bg-gray-100' : ''
                        }`}
                      >
                        {choice.label}
                      </button>
